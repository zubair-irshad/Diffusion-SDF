...
wandb.init(
    project="DiffusionSDF",
    tags=["train_diffusion_uconditional", "nerf", "kld_save_2_shape"],
)
...
class Trainer(object):
    def __init__(
        ...
        self.model = GaussianDiffusion(
            model=DiffusionNet(**model_specs).to(device), **diffusion_specs
        ).to(device)
        ...
        self.training_iters = int(training_iters / 10)
        ...
        self.resume = (
            os.path.join(self.results_folder, "{}.pt".format(args.resume))
        ...
        if self.resume:
            self.step, self.model, self.opt, loss = load_model(
                self.model, self.opt, self.resume
            )
    ...
    #         self.pbar.set_description(f'loss: {loss.item():.4f}')
    ...
    def train(self):
        ...
        with tqdm(initial=self.step, total=self.training_iters) as pbar:
            ...
            while self.step < self.training_iters:
                ...
                if self.has_cond:
                    pc = perturb_point_cloud(
                        pc, self.perturb_pc, self.pc_size, self.crop_percent
                    ).cuda()
                else:
                    pc = None
                ...
                # sample time
                t = torch.randint(
                    0, self.model.num_timesteps, (self.batch_size,), device=device
                ).long()
                loss, xt, target, pred, unreduced_loss = self.model(
                    data, t, ret_pred_x=True, cond=pc
                )
                ...
                if self.step % self.print_freq == 0:
                    print(
                        "avg loss at {} iters: {}".format(
                            self.step, current_loss / self.print_freq
                        )
                    )
                    print(
                        "losses per time at {} iters: {}, {}, {}".format(
                            self.step, mean(loss_100), mean(loss_500), mean(loss_1000)
                        )
                    )
                    wandb.log({"Diffusion loss 100": mean(loss_100)})
                    wandb.log({"Diffusion loss 500": mean(loss_500)})
                ...
                if self.step != 0 and self.step % self.save_and_sample_every == 0:
                    save_model(
                        ...
                        self.opt,
                        loss.detach(),
                        "{}/{}.pt".format(self.results_folder, self.step),
                    )
        #             writer.flush()
        # writer.close()
