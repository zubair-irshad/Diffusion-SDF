...
class ModulationLoaderCustom(torch.utils.data.Dataset):
    def __init__(self, data_path, pc_path=None, split_file=None, pc_size=None):
        ...
        self.samples_per_epoch = 1000 * (len(self.folders))
    ...
    def __getitem__(self, index):
        ...
        modulation_filename = os.path.join(
            self.data_source, self.folders[train_idx], "latent_shape.pth"
        )
        # latent = torch.from_numpy(np.loadtxt(modulation_filename)).float()
        latent = torch.load(
            modulation_filename, map_location=torch.device("cpu")
        ).float()

        if self.conditional:
            pcd_file_name = os.path.join(
                self.pc_source, self.folders[train_idx], "pcd.csv"
            )
